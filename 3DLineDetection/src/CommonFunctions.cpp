...
void LineFunctions::lineFitting( int rows, int cols, std::vector<cv::Point> &contour, 
                                 double thMinimalLineLength, std::vector<std::vector<cv::Point2d> > &lines )
{
	// get straight strings from the contour
	double minDeviation = 6.0; // if largest distance between points and line segment is smaller than this deviation
	std::vector<std::vector<cv::Point> > straightString; // save fitted line segment

	subDivision(straightString, contour, 0, contour.size()-1, minDeviation, int(thMinimalLineLength));
	if ( !straightString.size() ) // if haven't gotten any line segments
	{
		return;
	}
	for ( int i=0; i<straightString.size(); ++i ) // traverse every line segment
	{
		if ( straightString[i].size() < thMinimalLineLength ) // ignore short line sements 
		{
			continue;
		}

		// [0] indicate horizontal or vertical, [1] slope, [2] intercept
		std::vector<double> parameters( 4 ); 
		double maxDev = 0.0; // maximal deviation from line segment
		//bool isOK = lineFittingLS( straightString[i], parameters, maxDev );
		// (first point of line segment, length of line segment, ???out, out)
		lineFittingSVD(&straightString[i][0], straightString[i].size(), parameters, maxDev);
		//if ( isOK )
		{
			double k = parameters[1];
			double b = parameters[2];
			int lineLen = straightString[i].size(); // num of points that this straight line contains

			double xs = 0, ys = 0, xe = 0, ye = 0;
			if ( ! parameters[0] )  // horizontal
			{
				// coordinate of line's start point 
				xs = straightString[i][0].x;
				ys = k * xs + b;
				// coordinate of line's end point 
			...
			if ( !( xs==xe && ys==ye ) ) // if not one point
			{
	...
	} // endfor: we have saves all straight line segment 
}

void LineFunctions::subDivision( std::vector<std::vector<cv::Point> > &straightString, std::vector<cv::Point> &contour, 
                                 int first_index, int last_index, double min_deviation, int min_size )
{
	int clusters_count = straightString.size(); // num of line segments fitted and found, unused
	...
	int max_pixel_index = 0; // index of point that is most distant to line constructed by endpoints
	double max_deviation = -1.0; // distance between most distant point and line constructed by endpoints

	// i can loop from 0 to size
	for (int i=first_index, count=contour.size(); i!=last_index; i=(i+1)%count)
	{
		cv::Point current = contour[i];
		// 2d determinant is area.
		double deviation = static_cast<double>( abs( ((current.x - first.x) * (first.y - last.y)) + 
		                                             ((current.y - first.y) * (last.x - first.x)) ) );
	...
	max_deviation /= length; // distance between most distant point and line constructed by endpoints
	...
	if ((max_deviation>=min_deviation) &&                         // this contour can't be considered as a line
	    ((max_pixel_index - first_index + 1) >= half_min_size) && // we can cut this contour into 2 segments
			((last_index - max_pixel_index + 1) >= half_min_size))    // 
	{
	...
	else // this contour can be considered as a line
	{
		// this line segment is long enough
		if ( last_index - first_index > min_size )
		{
			std::vector<cv::Point> straightStringCur; // save all points between first and last index
			for ( int i=first_index; i<last_index; ++i )
			{
				straightStringCur.push_back(contour[i]); 
			}
			straightString.push_back(straightStringCur); // save this line segment
...
void LineFunctions::lineFittingSVD(cv::Point *points, int length, std::vector<double> &parameters, double &maxDev)
{
	...
	h_mean *= ( 1.0 / length ); // center point coordinate
	...
	cv::Matx21d normal = h_cov_evectors.row(1).t(); // ??? why select the middle row

	// ??? why distinguish horizontal and vertical
	...
	// maximal deviation from line segment
	maxDev = 0;
	for( int i = 0; i < length; ++i )
	{
		cv::Matx21d hi = cv::Matx21d( points[i].x, points[i].y );
		...
		double disOrtho = sqrt(dis2 - disNormal*disNormal); // distance from point to line
		if ( disOrtho > maxDev )
		{
			maxDev = disOrtho;
		}
...
void PCAFunctions::Ori_PCA( PointCloud<double> &cloud, int k, std::vector<PCAInfo> &pcaInfos, double &scale, double &magnitd )
{
	...
	// 2. knn search
	size_t *out_ks = new size_t[pointNum]; // save num of neighbnors
	size_t **out_indices = new size_t *[pointNum]; // save index of neighbor of every point
#pragma omp parallel for
	for (int i=0; i<pointNum; ++i)
	{
		...
		out_indices[i]= new size_t[k]; // malloc to save index of k neighbors of i-th point

		nanoflann::KNNResultSet<double> resultSet(k); // we should find k neibor points
		resultSet.init(out_indices[i], dis_temp );
		// (save result, this point, *)
	...
	for ( int i = 0; i < pointNum; ++i ) 
	{
		int ki = out_ks[i]; // num of neighbors 
		...
		// eigenvector
		cv::Matx33d h_cov_evectors; // eigen vector
		cv::Matx31d h_cov_evals;    // eigen values
		cv::eigen( h_cov, h_cov_evals, h_cov_evectors );
		...
		int idx = out_indices[i][3]; // index of 4-th neighbor
		dx = cloud.pts[idx].x - cloud.pts[i].x;
		dy = cloud.pts[idx].y - cloud.pts[i].y;
		dz = cloud.pts[idx].z - cloud.pts[i].z;
		double scaleTemp = sqrt(dx*dx + dy*dy + dz*dz); // distance as scale
...
{
	int i, j;
	int k = pointData.size(); // num of points 
	double a = 1.4826; // coeffient to define error band
	double thRz = 2.5; // threshold of error distance

	// 
	pcaInfo.idxIn.resize( k );
	cv::Matx31d h_mean( 0, 0, 0 ); // center of these points
	for( i = 0; i < k; ++i )
	{
		pcaInfo.idxIn[i] = i;
...
void PCAFunctions::MCMD_OutlierRemoval( std::vector<std::vector<double> > &pointData, PCAInfo &pcaInfo )
{
	double a = 1.4826; // coeffient to define error band
	double thRz = 2.5; // threshold of error distance
	int num = pcaInfo.idxAll.size(); // num of points
	...
	std::vector<double> ODs( num ); // save distance from point to plane
	for( int j = 0; j < num; ++j )
	{
	...
	double median_OD = meadian( sorted_ODs ); // median distance
	std::vector<double>().swap( sorted_ODs ); // delete this vector and release vector's memory 
