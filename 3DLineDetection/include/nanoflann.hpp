...
class KNNResultSet
{
  IndexType *indices;  // index ???
  DistanceType *dists; // distance ???
  CountType capacity;  // num of points that we should find ???
  CountType count;     // num of points that we have found ???
 ...
 public:
  ...
  inline void init(IndexType *indices_, DistanceType *dists_)
  {
    ...
    if (capacity)
    {
      dists[capacity - 1] = (std::numeric_limits<DistanceType>::max)(); // max
    }
  }
...
{
  size_t size;
  size_t read_cnt = fread(&size, sizeof(size_t), 1, stream); // a indicator to check file operation
  if (read_cnt != 1)
  {
    throw std::runtime_error("Cannot read from file");
  }
...
class KDTreeBaseClass
{
 public:
  ...
  void middleSplit_(Derived &obj, IndexType *ind, IndexType count, IndexType &index,
  ...
  /**
		 ...
		 */
  void planeSplit(Derived &obj, IndexType *ind, const IndexType count, int cutfeat,
                  DistanceType &cutval, IndexType &lim1, IndexType &lim2)
  {
    ...
    for (;;)
    {
      ...
      if (left > right || !right) // "!right" was added to support unsigned Index types
      {
        break;
      }
...
struct KDTreeEigenMatrixAdaptor
{
  ...
  KDTreeEigenMatrixAdaptor(const MatrixType &mat, const int leaf_max_size = 10) :
    m_data_matrix(mat)
  {
